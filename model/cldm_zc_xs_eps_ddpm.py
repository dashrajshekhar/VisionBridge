...
class ControlLDM(LatentDiffusion):
    ...
    def q_sample(self, x_start, t, z_c, noise=None):
        ...
        return (extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape)* x_start 
             ...
             * (noise + (extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x_start.shape) / ((extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, add_steps_tensor, x_start.shape))** 2)) * (z_c - x_start)))
    ...
    def predict_start_from_noise(self, x_t, t, z_c, noise):
        ...
        if (~is_add_steps).any():
            add_steps_tensor = torch.full_like(t, self.add_steps)
            pred_x0[~is_add_steps] = ((1 / (1. - (((extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape))** 2)[~is_add_steps] / (((extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, add_steps_tensor, x_t.shape))** 2)[~is_add_steps])))) *  extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.shape)[~is_add_steps]
               * (x_t[~is_add_steps] - extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)[~is_add_steps] * (noise[~is_add_steps] + ((extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape)[~is_add_steps] / ((extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, add_steps_tensor, x_t.shape))** 2))[~is_add_steps]) * z_c[~is_add_steps])))
    ...
    def sample_log(self, cond, un_cond, steps):
        ...
        samples, intermediates= sampler.sample(
            ...
            unconditional_conditioning=un_cond, eta=1.0
        )
        return samples
