...
from .ddim_zc import DDIMSampler
...
from ldm.util import log_txt_as_img, exists, instantiate_from_config, default, combine_text_as_img
...
class CDDM(nn.Module):
    ...
    def forward(self, x, hint, timesteps, context, base_model, **kwargs):
        ...
        for module_base, module_ctr in zip(base_model.input_blocks, self.control_model.input_blocks):
            ...
            hs_ctr.append(h_ctr)
            # h_ctr = h_ctr, h_base, next(it_enc_convs_in), self.infusion2control, emb
...
class ControlLDM(LatentDiffusion):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.perceptual_loss = instantiate_from_config(lossconfig)
    ...
    def instantiate_first_stage(self, config):
        ...
        # self.first_stage_model.train = True
    ...
    def apply_condition_compress(self, control, stream_path, H, W):
        ...
        out = self.preprocess_model.compress(ref, ref)
        print(f"compress_time: {time.time() - compress_time:.2f}s")
        shape = out["shape"]
    ...
    def apply_condition_decompress(self, stream_path):
        ...
        print(f'decompress_feature_time: {time.time() - decompress_time:.2f}s')
        return c_latent
    
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        # x, c = super().get_input(batch, self.first_stage_key, bs=bs, *args, **kwargs) 
        x_control = batch['jpg'] # k: key, 'jpg'
        if bs is not None:
            x_control = x_control[:bs]
        if len(x_control.shape) == 3:
            x_control = x_control[..., None]
        x_control = x_control.to(memory_format=torch.contiguous_format).float()
        with torch.no_grad():
            control = self.encode_first_stage(x_control).mode() * self.scale_factor
            x = control
        ...
        c_latent, likelihoods, q_likelihoods = self.apply_condition_encoder(control, control)
        ...
        return x, dict(c_crossattn=[c], c_latent=[c_latent], bpp=bpp, q_bpp=q_bpp, control=[control], ori=[batch['jpg']])
    
    # @torch.no_grad()
    def q_sample(self, x_start, t, z_c, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        add_steps_tensor = torch.full_like(t, self.add_steps)
        return (extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape)* x_start 
             +  extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) 
             * (noise + (1. /(extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, add_steps_tensor, x_start.shape))) * (z_c - x_start)))


    def predict_start_from_noise(self, x_t, t, z_c, noise):
        is_add_steps = (t == self.add_steps)
        pred_x0 =  z_c.clone()
        if (~is_add_steps).any():
            add_steps_tensor = torch.full_like(t, self.add_steps)
            pred_x0[~is_add_steps] = ((1 / (1. - ((extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape))[~is_add_steps] / ((extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, add_steps_tensor, x_t.shape))[~is_add_steps])))) *  extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.shape)[~is_add_steps]
               * (x_t[~is_add_steps] - extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)[~is_add_steps] * (noise[~is_add_steps] + (1. / ((extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, add_steps_tensor, x_t.shape)))[~is_add_steps]) * z_c[~is_add_steps])))
        return pred_x0
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        if cond['c_latent'] is None:
            ...
            eps = self.control_model(x=x_noisy, timesteps=t, context=cond_txt, hint=cond_hint, base_model=diffusion_model)
            return eps
    ...
    def log_images(self, batch, sample_steps=5, bs=2):
        ...
        log["original_image"] = (self.decode_first_stage(z) + 1) / 2
        log["control_image"] = (self.decode_first_stage(c_latent) + 1) / 2
        ...
        samples = self.sample_log(
            ...
            un_cond={"c_crossattn": [uc_cross], "c_latent": [c_latent]},
            steps=sample_steps
        )
    ...
    @torch.no_grad()
    def sample_log(self, cond, un_cond, steps):
        sampler = DDIMSampler(self)
        b, c, h, w = cond["c_latent"][0].shape
        shape = (self.channels, h, w)
    
        samples, intermediates= sampler.sample(
            steps, b, shape, cond, unconditional_guidance_scale=6.0,
            unconditional_conditioning=un_cond, eta=0
        )
        return samples
    ...
    def forward(self, x, c, *args, **kwargs):
        t = torch.randint(0, self.add_steps + 1, (x.shape[0],), device=self.device).long()
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        noise = default(noise, lambda: torch.randn_like(x_start))
        add_steps_tensor = torch.full_like(t, self.add_steps)
        x_noisy = self.q_sample(x_start=x_start, t=t, z_c=cond['c_latent'][0], noise=noise)
        model_output = self.apply_model(x_noisy, t, cond)
        if self.pred_x_start:
            pred_x0 = self.predict_start_from_noise(x_noisy, t=t, z_c=cond['c_latent'][0], noise=model_output)
        ...
        # print(f"Loss simple: {loss_simple}")
        ...
        if self.pred_x_start:
            rec = self.decode_first_stage(pred_x0)
        else:
            rec = self.decode_first_stage(c_latent)

        x_ori = cond['ori'][0]
        rec_loss, p_loss, perceptual_loss = self.perceptual_loss(rec, x_ori)

        loss_guide = self.get_loss(c_latent, x_start)

        loss_dict.update({f'{prefix}/l_guide': loss_guide.mean()})
        loss_dict.update({f'{prefix}/rec_loss': rec_loss.mean()})
        loss_dict.update({f'{prefix}/perceptual_loss': p_loss.mean()})
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        # print(f"Batch {batch_idx} received, optimizer_idx: {optimizer_idx}")
        if optimizer_idx == 0:
            ...
            # print(1)
            # print(f"Loss: {loss.item()}, Loss Dict: {loss_dict}")
