...
def initialize_prompt(tokenizer, token_embedding, args, device, initial_prompt_text=None):
    ...
    if initial_prompt_text is not None:
        ...
        # Ensure the prompt_ids length matches prompt_len, truncate or pad as necessary
...
def optimize_prompt_loop(model, tokenizer, token_embedding, all_target_features,compressed_feature, args, device, target_prompts):
    opt_iters = args.iter
    lr = args.lr
    ...
    # initialize prompt
    prompt_embeds, dummy_embeds, dummy_ids, prompt_len = initialize_prompt(tokenizer, token_embedding, args, device, target_prompts)
    ...
    for step in range(opt_iters):
        ...
        with torch.no_grad():
            ...
            logits_per_image, _ = model.forward_text_embedding(padded_embeds, dummy_ids, universal_target_features)#, compressed_feature)
        ...
        logits_per_image, _ = model.forward_text_embedding(padded_embeds, dummy_ids, target_features)#, compressed_feature)
...
def optimize_prompt(model, preprocess, args, device, target_images=None, compressed_img=None, prompt=None, target_prompts=None):
    ...
    compressed_feature = get_target_feature(model, preprocess, tokenizer_funct, device, target_images=compressed_img, target_prompts=target_prompts)

    # optimize prompt
    learned_prompt = optimize_prompt_loop(model, tokenizer, token_embedding, all_target_features,compressed_feature, args, device, prompt)
